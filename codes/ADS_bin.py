...
def get_VasLobeTemp(TemplateDir):
    ...
    vas_L1 = [ _.split('\t')[1] for _ in vas_contents]
    vas_L2 = [ _.split('\t')[3] for _ in vas_contents]
    vas_L1_name = [ _.split('\t')[5].split('\n')[0] for _ in vas_contents]

    vas_L1 = vas_L1[1:]
    vas_L2 = vas_L2[1:]
    vas_L1_name = vas_L1_name[1:]
...
#     print(np.max(vas_combine))
    ...
    with open(ReportTxt_pth, 'w') as f:
        f.write('area\tvolume\t\tvascular territory\tvolume\t\tvascular level2\tvolume\n')
        
        for idx in range(0,4):
            f.write(lobe_L1[idx]+'\t%d\t\t'% lobe_v_L1[idx] +vas_L1[idx]+'\t%d\t'% vas_v_L1[idx] + vas_L1_name[idx]+'\t'+vas_L2[idx]+'\t%d\n' % vas_v_L2[idx])
        
        for idx in range(4,31):
            f.write(lobe_L1[idx]+'\t%d\t\t'% lobe_v_L1[idx]+vas_L1[idx]+'\t%d\t'% vas_v_L1[idx]+ vas_L1_name[idx]+'\t\n')
            
        idx=31
        f.write('\t\t\t'+vas_L1[idx]+'\t%d\t'% vas_v_L1[idx] + vas_L1_name[idx]+'\t\n' )
        
        idx=32
        f.write('whole brain\t%d\n' % (Lesion_vol))
